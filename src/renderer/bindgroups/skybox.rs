...
impl<'a> HorizonBindGroup<'a> for SkyboxBindGroup {
    ...
    fn get_resources(device: &Device, resource_container: &mut BindingResourceContainer) {
        ...
        resource_container.textures[TextureTypes::Skybox] = Some(skybox_texture);
        resource_container.texture_views[TextureViewTypes::Skybox] = Some(skybox_texture_view);
        resource_container.buffers[BufferTypes::Skybox] = Some(skybox_buffer);
        resource_container.samplers[SamplerTypes::Skybox] = Some(skybox_sampler);
    }
}
