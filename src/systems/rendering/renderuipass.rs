...
impl<'a> System<'a> for RenderUIPass {
    ...
    fn run(
        ...
        let encoder = command_encoder.get_encoder();
        {
            ...
            let texture = binding_resource_container.texture_views[debug_ui.selected_texture_name]
                .as_ref()
                .unwrap();
        ...
        if debug_ui.texture_id.is_some() {
            ...
            debug_ui.texture_id = Some(egui_container.render_pass.egui_texture_from_wgpu_texture(
                &state.device,
                texture_view.as_ref().unwrap(),
                FilterMode::Linear,
            ));
